(function () {
  ...
  function manyBody() {
    ...
    function force(_) {
      var i, n = nodes.length, tree = quadtree(nodes, x, y$1).visitAfter(accumulate);
  ...
  function createTable(data) {
    ...
    updateHead.exit().remove();
    enterHead.merge(updateHead)
      .attr('class', 'th-sm')
      .text(d => d);
  ...
  function createChart(data, first=false) {
    const drag$1 = function(simulation) {
    ...
    const link$1 = select(".g-links").selectAll("line")
      .data(links)
      .join("line")
    ...
    const node = select(".g-nodes")
      ...
      .join("circle")
        ...
        .on("click", function(e, d) {
          const elems = selectAll('.g-nodes > circle').attr('fill', 'LightSteelBlue');
          select(e.currentTarget).attr('fill', 'steelblue');
          for (var i=0; i < data.nodes.length; i++) {
              const issubset = data.nodes[i].groups.every(function(val) {
                return data.nodes[d.index].groups.indexOf(val) >= 0;
              });
              if (issubset) {
                select(elems.nodes()[i]).attr('fill', 'steelblue');
              }
          }
  ...
  function createSummaryTable(data) {
    const tableA = select('#dtSummaryA');
    tableA.append('tbody');
